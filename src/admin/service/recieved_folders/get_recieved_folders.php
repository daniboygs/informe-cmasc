...
$db_table = "[dbo].[CarpetasRecibidas] cr INNER JOIN Usuario u ON cr.UsuarioID = u.UsuarioID
				...
				LEFT JOIN 
				( 
				SELECT MAX([Fecha]) AS 'acu_max_date' 
					,[NUC] 
				FROM [EJERCICIOS].[dbo].[AcuerdosCelebrados] 
				GROUP BY NUC 
				) a on cr.NUC = a.NUC";
		...
		while($row = sqlsrv_fetch_array( $result)){
			...
			array_push($return_data, array(
				...
				'agreement_date' => array(
					'name' => 'Fecha Acuerdo',
					'value' => formatRowDate($row['FechaAcuerdo'])
				),
				'investigation_date' => array(
					'name' => 'Fecha Acuerdo',
					'value' => formatRowDate($row['FechaInvestigacion'])
				),
				'validation_date' => array(
					'name' => 'Fecha Acuerdo',
					'value' => formatRowDate($row['FechaValidacion'])
				)
			));
		}
